...
func TestV1FrozenValues(t *testing.T) {
	t.Parallel()
	limits := V1Limits()
	if limits.FileDownloadBytes != 262144000 || limits.FileDownloadDeadlineMS != 120000 || ProductionFileOrigin != "https://files.slack.com" || FileDownloadPathPrefix != "/files-pri/" {
		t.Fatalf("file contract drifted: %+v", limits)
	}
